...
from app.voice.volume import get_voice_volume
...
class GladosVoiceService:
    ...
    def synthesize_wav(self, text: str) -> bytes:
        ...
        return _audio_to_wav_bytes(audio, sample_rate, volume=get_voice_volume())

    def synthesize_wav_for_client(self, text: str) -> bytes:
        """
        Synthesize wav for browser playback without server volume applied.

        Browser clients apply their own volume control to the audio element.
        ...
        settings = get_settings()
        sample_rate = int(getattr(tts, "sample_rate", settings.voice_sample_rate))
        return _audio_to_wav_bytes(audio, sample_rate, volume=1.0)
...
def _audio_to_wav_bytes(audio: Any, sample_rate: int, *, volume: float = 1.0) -> bytes:
    """
    ...
    Args:
        ...
        volume: Playback volume multiplier between 0.0 and 1.0.
    ...
    for sample in samples:
        clipped = max(-1.0, min(1.0, float(sample) * clamped_volume))
