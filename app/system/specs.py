...
def probe_cpu_temperature() -> float | None:
    """
    Return the host CPU temperature in Celsius when available.

    Returns:
        Temperature in Celsius, or None when the host does not expose it.
    """
...
def probe_cpu_throttled() -> bool | None:
    """
    Return whether the CPU is currently throttled on Raspberry Pi hosts.

    Returns:
        True when throttling is active, False when not, or None when unknown.
    """
...
def serialize_system_metrics() -> dict[str, float | bool | None]:
    """
    Return lightweight system metrics for API clients.

    Returns:
        Serializable CPU temperature and throttle state.
    """
    return {
